...
export type DocumentItem = { id: string; filename: string; sector: string; status: DocumentStatus; chunkCount: number; ingestedAt: string; allowedRoles: string[] }
export type ConflictItem = { id: number; documentIdA: string; documentIdB: string; score: number; snippetA: string; snippetB: string; status: string; note?: string }
export type UserItem = { id: number; username: string; roles: string[]; enabled: boolean }
export type QueryResponse =
  | { status: 'KNOWLEDGE'; answer: string; sourceIds: string[]; conversationId: string }
  | { status: 'TICKET_SUGGESTED'; ticketSuggestion: { category: string; subject: string; description: string; requester: string; tags?: string[] }; conversationId: string }
...
api.interceptors.request.use((config) => { const token = sessionStorage.getItem('ragkb-token'); if (token) config.headers.Authorization = `Bearer ${token}`; return config })
api.interceptors.response.use(undefined, (error) => { if (error.response?.status === 401) { sessionStorage.removeItem('ragkb-session'); sessionStorage.removeItem('ragkb-token'); if (location.pathname !== '/login') location.assign('/login') } return Promise.reject(error) })

export const backend = {
  login: (username: string, password: string) => api.post<{ token: string; expiresIn: number }>('/api/auth/login', { username, password }),
  query: (question: string, conversationId?: string) => api.post<QueryResponse>('/api/query', { question, conversationId }),
  ...
  updateDocument: (id: string, data: { action: 'ARCHIVE' | 'REACTIVATE'; allowedRoles?: string[] }) => api.patch<DocumentItem>(`/api/documents/${id}`, data),
